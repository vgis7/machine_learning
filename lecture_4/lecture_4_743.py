...
labels_df = pd.DataFrame(columns=['Label']) #Declares pandas dataframe which contains labels
labels_df = collect_labels(labels_df,mat_file['train5'],5) #Collects labels for each digit 5 image and saves it in the label dataframe
labels_df = collect_labels(labels_df,mat_file['train6'],6) #Collects labels for each digit 6 image and saves it in the label dataframe
labels_df = collect_labels(labels_df,mat_file['train8'],8) #Collects labels for each digit 8 image and saves it in the label dataframe
...
logisticRegr.fit(train,labels_df['Label'].astype('int')) #Fits the logistic regression model
...
calculate_accuracy(predictions,labels_df) #Calls the function.

#_________________________SCATTER PLOT_____________________________#
print("[INFO]: Plotting...")
fig, ax = plt.subplots()
index_5_begin = labels_df.loc[labels_df['Label']==5.0].index[0] #Locates the first index of the label 5 in the columns 'label' in the dataframe of labels. This enables the possibility of plotting the x,y positions of the dimentional reduced digits of 5
index_6_begin = labels_df.loc[labels_df['Label']==6.0].index[0] #Locates the first index of the label 6 in the columns 'label' in the dataframe of labels. This enables the possibility of plotting the x,y positions of the dimentional reduced digits of 6
index_8_begin = labels_df.loc[labels_df['Label']==8.0].index[0] #Locates the first index of the label 8 in the columns 'label' in the dataframe of labels. This enables the possibility of plotting the x,y positions of the dimentional reduced digits of 8
